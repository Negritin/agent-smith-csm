...
- Stripe webhook surface: `scripts/check-stripe-surface.sh` valida que
  `/api/webhooks/stripe` esta exposto e rejeita payload sem assinatura antes de
  processar eventos.
...
- Runtime check: `scripts/check-runtime.sh` passa, validando env core, env
  ...
  webhooks Stripe/WhatsApp. O login admin entra como check opcional quando
...
- `/opt/agent-smith/scripts/check-stripe-surface.sh`
