-- Sprint 2: public widget RPCs scoped to agent/company/session.
-- get_widget_messages_scoped requires an internal BFF proof signed with the
-- private.app_runtime_secrets. Direct anon callers with only
-- session_id/company_id/agent_id must not be able to read messages.
--
-- Operational provisioning required after deploying this migration:
...
CREATE OR REPLACE FUNCTION public.get_widget_agent_public(p_agent_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DROP FUNCTION IF EXISTS public.get_widget_messages_scoped(text, uuid, uuid, timestamptz);
DROP FUNCTION IF EXISTS public.get_widget_messages_scoped(text, uuid, uuid);
...
REVOKE ALL ON FUNCTION public.get_widget_agent_public(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_widget_messages_scoped(text, uuid, uuid, text, bigint, text) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.get_widget_agent_public(uuid) TO anon, authenticated;
