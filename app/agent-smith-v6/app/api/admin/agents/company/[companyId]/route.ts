...
export async function GET(
  ...
  try {
    ...
    if (!response.ok) {
      log.warn('[Admin Agents API] Backend returned non-success', { status: response.status });
    ...
    log.error('[Admin Agents API] Error', errorLogFields(error));
    return apiError('Internal server error', {
      cause: error,
      logMessage: '[Admin Agents API] Request failed',
      request,
      status: 500,
    });
  }
