...
/**
 * GET /api/admin/metrics/summary — 6 cards (SPEC §2.2).
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin.rpc('rpc_metrics_summary', {
      p_company_id: companyId,
      p_start: start,
      p_end: end,
    });
    if (error) {
      return apiError('Erro ao carregar métricas', {
        cause: error,
        logMessage: '[METRICS] summary RPC failed',
        request,
        status: 500,
      });
    }
    ...
    return apiError('Erro ao carregar métricas', {
      cause: err,
      logMessage: '[METRICS] summary unexpected error',
      request,
      status: 500,
    });
  }
