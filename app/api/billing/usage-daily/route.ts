...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL('/api/billing/usage-daily', BACKEND_URL);
    ...
    if (!response.ok) {
      log.warn('[Billing Usage Daily API] Backend returned non-success', {
        status: response.status,
      });
      return apiError('Erro ao carregar uso diário', { request, status: response.status });
    }

    return NextResponse.json(await response.json());
  } catch (error: unknown) {
    log.error('[Billing Usage Daily API] Error', errorLogFields(error));
    return apiError('Erro interno', {
      cause: error,
      logMessage: '[Billing Usage Daily API] Request failed',
      request,
      status: 500,
    });
  }
