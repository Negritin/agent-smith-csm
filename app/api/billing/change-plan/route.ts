...
async function getCompanyIdFromSession(): Promise<string | null> {
  try {
    ...
    log.error('[ChangePlan] Error getting company_id', errorLogFields(error));
    return null;
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      log.warn('[ChangePlan API] Backend returned non-success', { status: response.status });
      return apiError('Erro ao alterar plano', {
        request,
    ...
    log.error('[ChangePlan API] Error', errorLogFields(error));
    return apiError('Erro interno', {
      cause: error,
      logMessage: '[ChangePlan API] Request failed',
      request,
      status: 500,
    });
  }
