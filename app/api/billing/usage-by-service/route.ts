...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL('/api/billing/usage-by-service', BACKEND_URL);
    ...
    if (!response.ok) {
      log.warn('[Billing Usage Service API] Backend returned non-success', {
        status: response.status,
      });
      return apiError('Erro ao carregar uso por serviço', {
        request,
        status: response.status,
      });
    }
    ...
    log.error('[Billing Usage Service API] Error', errorLogFields(error));
    return apiError('Erro interno', {
      cause: error,
      logMessage: '[Billing Usage Service API] Request failed',
      request,
      status: 500,
    });
  }
