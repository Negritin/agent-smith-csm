...
/**
 * GET /api/admin/metrics/attendance — aba "Atendimentos" (SPEC §4).
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin.rpc('rpc_metrics_attendance', {
      p_company_id: companyId,
      p_start: start,
      p_end: end,
    });
    if (error) {
      return apiError('Erro ao carregar métricas de atendimento', {
        cause: error,
        logMessage: '[METRICS] attendance RPC failed',
        request,
        status: 500,
      });
    }
    ...
    return apiError('Erro ao carregar métricas de atendimento', {
      cause: err,
      logMessage: '[METRICS] attendance unexpected error',
      request,
      status: 500,
    });
  }
