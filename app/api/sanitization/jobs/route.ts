...
async function resolveCompanyId(
    ...
    try {
        ...
        log.error('[Sanitization Jobs] Error resolving company_id', errorLogFields(error));
        return null;
    }
}
...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!response.ok) {
            log.warn('[Sanitization Jobs API] Backend returned non-success', {
                status: response.status,
            });
            return apiError('Erro ao listar jobs de sanitização', {
                request,
                status: response.status,
            });
        }
        ...
        log.error('[Sanitization Jobs API] Error', errorLogFields(error));
        return apiError('Erro interno', {
            cause: error,
            logMessage: '[Sanitization Jobs API] Request failed',
            request,
            status: 500,
        });
    }
